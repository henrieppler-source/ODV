...
from .file_service import append_metadata_history, detect_document_type, make_normalized_archive_filename
...
class AdminDetailManagerMixin:
    def show_selected_admin_details(self) -> None:
        ...
        if not item:
            return
        self._loading_admin_details = True
        is_missing_odv_entry = bool(item.get("_missing_odv_entry"))
        try:
            api_item = self.api_get_document_item(str(item.get("upload_id", ""))) if self.api_token and not is_missing_odv_entry else None
            if api_item:
        ...
        if can_edit and self.api_token and item.get("upload_id") and not is_missing_odv_entry:
            try:
